...
(async function () {
  ...
  // Mount upload queue AFTER template() has rendered #uq-host into the DOM.
  ...
  function enqueueRecording(blob, durationSec) {
    ...
    // Show the just-recorded clip so the operator can verify capture.
    // All actions (Download, Open Drive, Retry) live on the queue panel below.
  ...
  function template() {
    return `
        ...
        <div>
          ...
          <div class="last-recorded" id="last-recorded">
            ...
            <div class="dim mt-sm" style="font-size:.78rem;">
              Upload runs in the background — see the <b>Upload Queue</b> below for status,
              local download, or Drive link.
            </div>
          </div>
        </div>
      ...
      <!-- upload queue panel (full-width, below the grid) -->
      <div id="uq-host"></div>
    `;
  }
})();
