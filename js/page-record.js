...
(async function () {
  ...
  const uploadBox  = UI.$('#upload-stage');
  const uploadBar  = UI.$('#upload-bar');
  const uploadPct  = UI.$('#upload-pct');
  const uploadName = UI.$('#upload-name');
  ...
  async function uploadRecording(blob, durationSec) {
    ...
    try {
      const result = await Uploader.upload(blob, meta, p => {
        const pct = Math.round(p * 100);
        uploadBar.style.width = pct + '%';
        uploadPct.textContent = pct + '%';
      });
      UI.toast('Upload complete', fileName, 'ok');

      // show recorded clip preview
      lastBox.classList.add('show');
      lastVideo.src = URL.createObjectURL(blob);
      if (result.driveUrl) {
        openDrive.href = result.driveUrl;
        openDrive.classList.remove('hidden');
      }
      lockFields(false);
      // pre-fill remarks empty for next recording
      UI.$$('#dyn-fields [name]').forEach(i => {
        if (['remarks','orderId','returnId','customerName'].includes(i.name)) i.value = '';
      });
    } catch (err) {
      UI.toast('Upload failed', err.message || 'Try the Retry button.', 'danger', 6000);
      // offer retry
      uploadBox.innerHTML += `
        <div class="mt-sm">
          <button class="btn primary" id="retry-btn">Retry upload</button>
        </div>`;
      UI.$('#retry-btn').addEventListener('click', () => uploadRecording(blob, durationSec));
    }
  }
      ...
      <div class="record-grid">
        ...
        <div>
          ...
          <div class="upload-stage" id="upload-stage">
            <div class="label">
              <span><b id="upload-name">file.webm</b></span>
              <span class="mono" id="upload-pct">0%</span>
            </div>
