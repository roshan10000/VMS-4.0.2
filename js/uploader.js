...
const Uploader = (() => {
  ...
  /** Returns the in-memory Blob for a job, or null if it's been freed
   *  (which happens after a successful upload, or after a page reload). */
  function getBlob(id) {
    const j = jobs.get(id);
    return j && j.blob ? j.blob : null;
  }
  ...
  /* ---- the actual upload over fetch() ----------------------------------
   * Why fetch and not XHR-with-progress?
   *   Apps Script Web Apps don't send CORS response headers. To avoid a
   *   browser preflight, the request must qualify as a "simple request":
   *     - method: POST
   *     - content-type: text/plain / application/x-www-form-urlencoded /
   *                     multipart/form-data
   *     - no custom headers, NO upload progress listener (this triggers
   *       preflight on XHR for cross-origin requests).
   *   So we use fetch() and fake smooth progress with a timed estimator
   *   based on file size and average throughput.
   * --------------------------------------------------------------------- */
  async function uploadOnce(job) {
    ...
    // Estimated upload time based on a conservative 250 KB/sec throughput.
    // The progress bar walks from 10% → 90% over that window, so the user
    // sees forward motion even though fetch() doesn't expose real progress.
    const estMs = Math.max(2000, Math.round((body.length / 250000) * 1000));
    const startedAt = Date.now();
    const tick = setInterval(() => {
      const elapsed = Date.now() - startedAt;
      const frac = Math.min(0.90, 0.10 + (elapsed / estMs) * 0.80);
    ...
    try {
      ...
      if (!json.ok) throw new Error(json.error || 'Upload rejected');
      return json.data;
    } finally {
      clearInterval(tick);
    }
  ...
  return {
    ...
    getJobs, getBlob, hasPending, subscribe,
    ONESHOT_MAX_BYTES,
  };
})();
